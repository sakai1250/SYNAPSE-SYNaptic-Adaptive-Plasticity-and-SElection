...
def select_learner_units_resnet(network: Any, stable_selection_perc, train_episode: Any, episode_index: int) -> Any:
    ...
    # =================================================================
    # SYNAPSE: 新しく学習者(Learner)になったニューロンの所属タスクを記録
    # =================================================================
    for i, (ranks, name) in enumerate(network.unit_ranks[1:], 1):
        # ランクが 1 (Learner) になったニューロンを抽出
        new_learner_indices = np.where(ranks == 1)[0]
        for neuron_idx in new_learner_indices:
            # まだ記録されていないニューロンのみ記録（一度生まれたタスクは変わらない）
            if neuron_idx not in network.neuron_birth_task[name]:
                network.neuron_birth_task[name][neuron_idx] = episode_index
                print(f"  [SYNAPSE LOG] Neuron {neuron_idx} in layer {name} is assigned to task {episode_index}.")
    # =================================================================
